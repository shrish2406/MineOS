...
const ViewShotComponent = forwardRef(function ViewShot(props, ref) {
    ...
    const firstLayoutPromise = useMemo(() => new Promise(resolve => {
    ...
    const capture = useCallback(() => firstLayoutPromise
        .then(() => {
        if (!rootRef.current)
    ...
    const syncCaptureLoop = useCallback((mode) => {
        cancelAnimationFrame(rafRef.current);
        if (mode === "continuous") {
            let previousCaptureURI = "-";
            const loop = () => {
                rafRef.current = requestAnimationFrame(loop);
                if (previousCaptureURI === lastCapturedURIRef.current)
                    return;
