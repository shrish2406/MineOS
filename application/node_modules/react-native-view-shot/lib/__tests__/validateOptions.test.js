...
describe("validateOptions (via captureRef)", () => {
    ...
    it("passes valid options through", async () => {
        ...
        expect(mockCaptureRef).toHaveBeenCalledWith(42, expect.objectContaining({
            format: "jpg",
            quality: 0.8,
            result: "base64",
            width: 100,
    ...
    it("resets invalid quality to default", async () => {
        ...
        expect(mockCaptureRef).toHaveBeenCalledWith(42, expect.objectContaining({ quality: 1 }));
        warnSpy.mockRestore();
    });
    ...
    it("resets invalid format to png", async () => {
        const warnSpy = jest.spyOn(console, "warn").mockImplementation();
        await captureRef(42, { format: "bmp" });
        expect(mockCaptureRef).toHaveBeenCalledWith(42, expect.objectContaining({ format: "png" }));
        warnSpy.mockRestore();
    });
    it("resets invalid result to tmpfile", async () => {
        const warnSpy = jest.spyOn(console, "warn").mockImplementation();
        await captureRef(42, { result: "invalid" });
        expect(mockCaptureRef).toHaveBeenCalledWith(42, expect.objectContaining({ result: "tmpfile" }));
        warnSpy.mockRestore();
    });
    it("warns in __DEV__ when options are invalid", async () => {
        const warnSpy = jest.spyOn(console, "warn").mockImplementation();
        await captureRef(42, { quality: 5, width: -1 });
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("bad options"));
        warnSpy.mockRestore();
    });
    it("accepts data-uri result", async () => {
        await captureRef(42, { result: "data-uri" });
        expect(mockCaptureRef).toHaveBeenCalledWith(42, expect.objectContaining({ result: "data-uri" }));
    });
});
describe("captureScreen", () => {
    let captureScreen;
    let mockCaptureScreen;
    beforeEach(() => {
        jest.resetModules();
    ...
    it("calls native captureScreen with validated options", async () => {
        ...
        expect(mockCaptureScreen).toHaveBeenCalledWith(expect.objectContaining({
            format: "jpg",
            quality: 0.5,
        }));
    });
...
describe("releaseCapture", () => {
    ...
    it("warns and does not call native when uri is not a string", () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("Invalid argument"));
        warnSpy.mockRestore();
    });
});
//# sourceMappingURL=validateOptions.test.js.map